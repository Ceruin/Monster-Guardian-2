...
namespace AICommand
{
    internal static class OpenAIUtil
    {
        ...
        public static string InvokeChat(string prompt)
        {
            ...
            // Create JSON string from chat request body
            string jsonRequestBody = CreateChatRequestBody(prompt);

            // Create a new UnityWebRequest
            using var post = new UnityWebRequest(OpenAI.Api.Url, UnityWebRequest.kHttpVerbPOST)
            {
                uploadHandler = new UploadHandlerRaw(Encoding.UTF8.GetBytes(jsonRequestBody)),
